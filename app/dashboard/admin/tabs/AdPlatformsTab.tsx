      ...
      {showForm && (
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-4 mb-6 flex gap-3 items-end flex-wrap">
          <div>
            ...
            <input value={name} onChange={(e) => { setName(e.target.value); setSlug(slugify(e.target.value)) }} placeholder="e.g. Facebook" className="bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm w-48 focus:outline-none focus:ring-2 focus:ring-red-500" />
          </div>
          <div>
            <label className="block text-slate-400 text-xs mb-1.5">Slug</label>
            <input value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="e.g. facebook" className="bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm w-48 focus:outline-none focus:ring-2 focus:ring-red-500" />
          </div>
