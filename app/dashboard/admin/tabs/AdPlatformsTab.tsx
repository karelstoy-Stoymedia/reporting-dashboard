  ...
  return (
    <AdminCard>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-gray-900 font-semibold text-lg">Ad Platforms</h2>
          <p className="text-gray-500 text-sm mt-0.5">e.g. Facebook, Google, TikTok</p>
        </div>
        <button onClick={() => setShowForm(!showForm)} className="bg-red-600 hover:bg-red-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">+ Add Platform</button>
      </div>
      ...
      {showForm && (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6 flex gap-3 items-end flex-wrap">
          <div>
            ...
            <input value={name} onChange={(e) => { setName(e.target.value); setSlug(slugify(e.target.value)) }} placeholder="e.g. Facebook" className={`${inputCls} w-48`} />
          </div>
          <div>
            <label className="block text-gray-600 text-xs font-medium mb-1.5">Slug</label>
            <input value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="e.g. facebook" className={`${inputCls} w-48`} />
          </div>
      ...
      {editItem && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-white border border-gray-200 rounded-xl p-6 w-full max-w-md shadow-xl">
            <h3 className="text-gray-900 font-semibold mb-4">Edit Ad Platform</h3>
