  ...
  return (
    <AdminCard>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-white font-semibold text-lg">Sales Reps</h2>
          <p className="text-slate-400 text-sm mt-0.5">Deactivated reps remain in historical data</p>
        </div>
        <button onClick={() => setShowForm(!showForm)} className="bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">+ Add Rep</button>
      </div>
      ...
      {showForm && (
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-4 mb-6 flex gap-3 items-end flex-wrap">
          <div>
            ...
            <input value={name} onChange={(e) => { setName(e.target.value); setSlug(slugify(e.target.value)) }} placeholder="e.g. Karel Stoy" className="bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm w-48 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>
          <div>
            <label className="block text-slate-400 text-xs mb-1.5">Slug</label>
            <input value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="e.g. karel_stoy" className="bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm w-48 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>
          <div>
            <label className="block text-slate-400 text-xs mb-1.5">Email (optional)</label>
            <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="rep@stoymedia.com" className="bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm w-48 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>
      ...
      {editItem && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 w-full max-w-md shadow-xl">
            <h3 className="text-white font-semibold mb-4">Edit Sales Rep</h3>
            <div className="space-y-4">
              ...
              <div>
                ...
                <input value={editEmail} onChange={(e) => setEditEmail(e.target.value)} className="w-full bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
              </div>
            </div>
          ...
          {items.map((s) => (
            <tr key={s.id}>
              ...
              <td className="py-3 flex gap-3">
                <button onClick={() => { setEditItem(s); setEditName(s.name); setEditSlug(s.slug); setEditEmail(s.email ?? '') }} className="text-indigo-400 hover:text-indigo-300 text-xs">Edit</button>
                <button onClick={() => handleToggle(s.id, s.is_active)} className="text-slate-400 hover:text-white text-xs">{s.is_active ? 'Deactivate' : 'Restore'}</button>
              </td>
            </tr>
          ))}
        </AdminTable>
