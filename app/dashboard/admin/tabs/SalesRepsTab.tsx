  ...
  return (
    <AdminCard>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setShowForm(!showForm)} className="bg-red-600 hover:bg-red-500 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">+ Add Rep</button>
      </div>
      ...
      {showForm && (
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-4 mb-6 flex gap-3 items-end flex-wrap">
          <div>
            ...
            <input value={name} onChange={(e) => { setName(e.target.value); setSlug(slugify(e.target.value)) }} placeholder="e.g. Karel Stoy" className="bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm w-48 focus:outline-none focus:ring-2 focus:ring-red-500" />
          </div>
          <div>
            <label className="block text-slate-400 text-xs mb-1.5">Slug</label>
            <input value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="e.g. karel_stoy" className="bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm w-48 focus:outline-none focus:ring-2 focus:ring-red-500" />
          </div>
          <div>
            <label className="block text-slate-400 text-xs mb-1.5">Email (optional)</label>
            <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="rep@stoymedia.com" className="bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm w-48 focus:outline-none focus:ring-2 focus:ring-red-500" />
          </div>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 w-full max-w-md shadow-xl">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input value={editEmail} onChange={(e) => setEditEmail(e.target.value)} className="w-full bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
              </div>
            </div>
          ...
          {items.map((s) => (
            <tr key={s.id}>
              ...
              <td className="py-3 flex gap-3">
                <button onClick={() => { setEditItem(s); setEditName(s.name); setEditSlug(s.slug); setEditEmail(s.email ?? '') }} className="text-red-400 hover:text-red-300 text-xs">Edit</button>
