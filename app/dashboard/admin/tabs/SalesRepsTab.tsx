      ...
      {showForm && (
        <div className="bg-gray-100 border border-gray-300 rounded-lg p-4 mb-6 flex gap-3 items-end flex-wrap">
          <div>
            ...
            <input value={name} onChange={(e) => { setName(e.target.value); setSlug(slugify(e.target.value)) }} placeholder="e.g. Karel Stoy" className="bg-white border border-slate-600 text-white rounded-lg px-3 py-2 text-sm w-48 focus:outline-none focus:ring-2 focus:ring-red-500" />
          </div>
          <div>
            <label className="block text-gray-500 text-xs mb-1.5">Slug</label>
            <input value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="e.g. karel_stoy" className="bg-white border border-slate-600 text-white rounded-lg px-3 py-2 text-sm w-48 focus:outline-none focus:ring-2 focus:ring-red-500" />
          </div>
          <div>
            <label className="block text-gray-500 text-xs mb-1.5">Email (optional)</label>
            <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="rep@stoymedia.com" className="bg-white border border-slate-600 text-white rounded-lg px-3 py-2 text-sm w-48 focus:outline-none focus:ring-2 focus:ring-red-500" />
          </div>
          ...
          <div className="bg-gray-100 border border-gray-300 rounded-xl p-6 w-full max-w-md shadow-xl">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input value={editEmail} onChange={(e) => setEditEmail(e.target.value)} className="w-full bg-white border border-slate-600 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
              </div>
            </div>
          ...
          {items.map((s) => (
            <tr key={s.id}>
              ...
              <td className="py-3 flex gap-3">
                ...
                <button onClick={() => handleToggle(s.id, s.is_active)} className="text-gray-500 hover:text-white text-xs">{s.is_active ? 'Deactivate' : 'Restore'}</button>
              </td>
            </tr>
          ))}
        </AdminTable>
