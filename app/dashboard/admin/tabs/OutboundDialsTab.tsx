...
export default function OutboundDialsTab() {
  ...
  return (
    <AdminCard>
      <div className="mb-6">
        <h2 className="text-gray-900 font-semibold text-lg">Outbound Dials</h2>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6 flex gap-3 items-end flex-wrap">
        <div>
          <label className="block text-gray-600 text-xs font-medium mb-1.5">Channel</label>
          <select value={channelId} onChange={(e) => setChannelId(e.target.value)} className={`${inputCls} w-48`}>
            <option value="">Select channel</option>
            {channels.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-gray-600 text-xs font-medium mb-1.5">Date</label>
          <input type="date" value={eventDate} onChange={(e) => setEventDate(e.target.value)} className={`${inputCls} w-44`} />
        </div>
        <div>
          <label className="block text-gray-600 text-xs font-medium mb-1.5">Dials Made</label>
          <input type="number" value={dials} onChange={(e) => setDials(e.target.value)} placeholder="e.g. 250" className={`${inputCls} w-32`} />
        </div>
        <div>
          <label className="block text-gray-600 text-xs font-medium mb-1.5">Connects</label>
          <input type="number" value={connects} onChange={(e) => setConnects(e.target.value)} placeholder="e.g. 18" className={`${inputCls} w-32`} />
        </div>
        <button onClick={handleSave} disabled={saving} className="bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white text-sm font-medium px-4 py-2 rounded-lg">{saving ? 'Saving...' : 'Save'}</button>
      ...
      {editItem && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-white border border-gray-200 rounded-xl p-6 w-full max-w-md shadow-xl">
            <h3 className="text-gray-900 font-semibold mb-4">Edit Dial Entry</h3>
            <p className="text-gray-500 text-sm mb-4">{editItem.outbound_channels?.name} — {editItem.event_date}</p>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-gray-600 text-xs font-medium mb-1.5">Dials Made</label>
                <input type="number" value={editDials} onChange={(e) => setEditDials(e.target.value)} className={`w-full ${inputCls}`} />
              </div>
              <div>
                <label className="block text-gray-600 text-xs font-medium mb-1.5">Connects</label>
                <input type="number" value={editConnects} onChange={(e) => setEditConnects(e.target.value)} className={`w-full ${inputCls}`} />
              </div>
            </div>
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-white border border-gray-200 rounded-xl p-6 w-full max-w-sm shadow-xl">
            ...
            <p className="text-gray-600 text-sm mb-6">This will permanently remove this dial entry.</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200">
                {['Channel', 'Date', 'Dials', 'Connects', 'Connect Rate', 'Actions'].map(h => (
            ...
            <tbody className="divide-y divide-gray-100">
              {entries.slice(0, 20).map((e) => (
                <tr key={e.id} className="hover:bg-gray-50">
                  <td className="py-3 pr-4 text-gray-900">{e.outbound_channels?.name}</td>
                  <td className="py-3 pr-4 text-gray-600">{e.event_date}</td>
                  <td className="py-3 pr-4 text-gray-900 font-medium">{e.dials_made.toLocaleString()}</td>
                  <td className="py-3 pr-4 text-gray-900 font-medium">{e.connects.toLocaleString()}</td>
                  <td className="py-3 pr-4 text-gray-500">{e.dials_made > 0 ? ((e.connects / e.dials_made) * 100).toFixed(1) : 0}%</td>
                  <td className="py-3 flex gap-3">
                    <button onClick={() => { setEditItem(e); setEditDials(String(e.dials_made)); setEditConnects(String(e.connects)) }} className="text-red-600 hover:text-red-700 text-xs font-medium">Edit</button>
                    <button onClick={() => setDeleteId(e.id)} className="text-gray-400 hover:text-red-600 text-xs">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
